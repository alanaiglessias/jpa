...
public class SetorServletListar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Setor> listaSetores = sControl.listarSetores();

        if ((listaSetores != null) && (listaSetores.size() > 0)) {
            request.setAttribute("lista", listaSetores);
            request.getRequestDispatcher("listarsetores.jsp").forward(request, response);
        } else {
            request.setAttribute("mensagem", "Nenhum registro cadastrado!!!!");
